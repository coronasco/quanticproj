...
const ProductManager = () => {
  ...
  return (
    <div className="p-4 m-4 md:m-6 border rounded-lg bg-gray-50 mb-6">
      ...
      <p className="mb-6 text-sm">Aggiungi prodotti per poi creare lista spese</p>
      <div className="flex flex-wrap lg:flex-nowrap gap-2 items-end">
            <div className="w-full">
                <label htmlFor="product" className="text-xs">Prodotto</label>
                <Input placeholder="Prodotto" id="product" value={name} onChange={(e) => setName(e.target.value)} />
            </div>
            <div className="w-full">
                <label htmlFor="price" className="text-xs">Prezzo</label>
                <Input type="number" id="price" placeholder="Prezzo" value={price} onChange={(e) => setPrice(e.target.value)} />
            </div>
            <div className="w-full">
                <label htmlFor="vat" className="text-xs">Iva</label>
                <Input type="number" id="vat" placeholder="Iva" value={vat} onChange={(e) => setVat(e.target.value)} />
            </div>
            <div className="w-full">
                <label htmlFor="shop" className="text-xs">Negozio</label>
                <Input placeholder="Negozio" id="negozio" value={store} onChange={(e) => setStore(e.target.value)} /> {/* 🔹 Magazin */}
            </div>
            <Button onClick={handleSaveProduct} className="mt-2 ">Salva Prodotto</Button>
        </div>
      
      <div className="mt-6">
        {
            products.length < 1 ? 
                <p className="text-sm text-gray-500 text-center">Non hai aggiunto nessun prodotto!</p> 
            :
                <Table>
